...
class ContextualBlockBifurcationConformerEncoder(AbsEncoder):
    ...
    def forward_train(
        ...
        # forward
        xs_chunk, mask_online, _, _, _, _, _ = self.encoders(
            xs_chunk, mask_online, False, xs_chunk
        )
        ys_chunk, mask_online, _, _, _, _, _ = self.encoders_aux1(
            xs_chunk, mask_online, False, xs_chunk
        )
        
        xs_chunk2, mask_online2, _, _, _, _, _ = self.encoders(
            xs_chunk2, mask_online2, False, xs_chunk2
        )
        ys_chunk2, mask_online2, _, _, _, _, _ = self.encoders_aux2(
            xs_chunk2, mask_online2, False, xs_chunk2
        )
    ...
    def forward_infer(
        ...
        for i in range(block_num):
            ...
            i#prev_addin_r = xs_pad.narrow(1, cur_hop, cur_hop+self.look_ahead)
        ...
        mask_online2 = xs_pad_r.new_zeros(
        ...
        # forward
        xs_chunk, mask_online, _, _, past_encoder_ctx, _, _ = self.encoders(
            xs_chunk, mask_online, True, past_encoder_ctx
        )
        ys_chunk, mask_online, _, _, past_encoder_ctx, _, _ = self.encoders_aux1(
            xs_chunk, mask_online, True, past_encoder_ctx
        )
        
        xs_chunk_r, mask_online2, _, _, past_encoder_ctx_r, _, _ = self.encoders(
            xs_chunk_r, mask_online2, True, past_encoder_ctx_r
        )
        ys_chunk_r, mask_online2, _, _, past_encoder_ctx_r, _, _ = self.encoders_aux2(
            xs_chunk_r, mask_online2, True, past_encoder_ctx_r
        )
