...
use_eou=false # true: <sos/eos>, false: <s> and </s> 
sos="<s>" # sos symbole
eos="</s>" # eos symbole
...
word_vocab_size=123 # Size of word vocabulary.
my_token_listdir=noinfo    # add by Jin Sakuma
...
use_transducer=false # add by Jin Sakuma
use_parallel_transducer=false # add by Jin Sakuma
use_dual_delay=false # add by Jin Sakuma
...
if [ "${my_token_listdir}" != noinfo ]; then
    token_listdir=data/${my_token_listdir}
fi
...
chartoken_list="${token_listdir}"/char/tokens2.txt
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        ...
        if "${use_k2}"; then
          ...
          if "${use_transducer}"; then
              asr_inference_tool="espnet2.bin.asr_transducer_inference"
          elif "${use_parallel_transducer}"; then
              asr_inference_tool="espnet2.bin.asr_parallel_transducer_inference"
          elif "${use_streaming}"; then
              asr_inference_tool="espnet2.bin.asr_inference_streaming"
          elif "${use_dual_delay}"; then
              asr_inference_tool="espnet2.bin.asr_inference_dual_delay_streaming"
